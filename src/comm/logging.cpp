...
void Logger::logv(int level, const char *file, int line, const char *func, const char *fmt...) {
    ...
    p += snprintf(p, limit - p, "%04d-%02d-%02d %02d:%02d:%02d.%03d %lx %s %s:%d ", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec,
                  static_cast<int>(now_tv.tv_usec) / 1000, (long) tid, levelStrs_[level], file, line);
    va_list args;
    va_start(args, fmt);
